<?php
/**
 * Rawayah functions and definitions
 *
 ...
 */
function rawayah_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on Rawayah, use a find and replace
		* to change 'rawayah' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'rawayah', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'rawayah' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'rawayah_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'rawayah_setup' );
 ...
 */
function rawayah_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'rawayah_content_width', 640 );
}
add_action( 'after_setup_theme', 'rawayah_content_width', 0 );
 ...
 */
function rawayah_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'rawayah' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'rawayah' ),
...
add_action( 'widgets_init', 'rawayah_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function rawayah_scripts() {
	wp_enqueue_style( 'rawayah-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'rawayah-style', 'rtl', 'replace' );

	wp_enqueue_script( 'rawayah-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'rawayah_scripts' );
